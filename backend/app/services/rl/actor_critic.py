"""Actor-Critic 网络：Actor 输出下一节点/充电决策，Critic 评估状态价值。"""
...
class ActorCritic:
    """基于 NumPy 的 Actor-Critic，支持 mamba_ppo / transformer_ppo / ppo 三种编码器。"""
    ...
    def act(
        ...
        if deterministic:
            ...
            probs = _softmax(logits / max(temperature, 0.1))
